...
-(void)loadDataSourceWithPage:(int)page{
    ...
    [[ShareWork sharedManager]queryFriendsWithMid:[AccountManager sharedAccountManager].loginModel.mid page:page size:REQUEST_PAGE_SIZE complete:^(BaseModel *model) {
...
(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        // 数据源也要相应删除一项
        FriendModel * model = self.dataSource[indexPath.row];
        UIAlertController * alert = [UIAlertController alertControllerWithTitle:@"提示"message:@"您确定要删除此好友?"preferredStyle:UIAlertControllerStyleAlert];
//
        ...
        [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            [[ShareWork sharedManager]delFriendWithMid:[AccountManager sharedAccountManager].loginModel.mid friend:model.mid complete:^(BaseModel *model) {
                if (model) {
