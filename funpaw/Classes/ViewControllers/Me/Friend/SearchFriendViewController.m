...
-(void)setupView{
    ...
    _topTextfield.placeholder = @"手机号/昵称/设备号";
...
-(void)loadDataSourceWithPage:(int)page{
    ...
    [[ShareWork sharedManager]searchPeopleWithMid:[AccountManager sharedAccountManager].loginModel.mid condition:_topTextfield.text page:page size:REQUEST_PAGE_SIZE complete:^(BaseModel *model) {
        if (model.list.count == 0) {
            [[AppUtil appTopViewController] showHint:@"未搜索到该用户"];
...
-(void)rightButtonTouch22:(UIButton *)sender{
    ...
    [[ShareWork sharedManager]addFriendRequsetWithMid:[AccountManager sharedAccountManager].loginModel.mid friend:model.mid complete:^(BaseModel *model) {
        if (model) {
