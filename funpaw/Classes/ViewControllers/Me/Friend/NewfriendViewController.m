...
-(void)isreaddd{
    [[ShareWork sharedManager]modifyFriendsMsgStatusWithMid:[AccountManager sharedAccountManager].loginModel.mid complete:^(BaseModel *model) {
        if (model) {
...
-(void)loadDataSourceWithPage:(int)page{
    [[ShareWork sharedManager]newFriendsMsgWithMid:[AccountManager sharedAccountManager].loginModel.mid page:page size:REQUEST_PAGE_SIZE complete:^(BaseModel *model) {
...
-(void)cellrightButtonTouch:(UIButton *)sender{
    ...
    [[ShareWork sharedManager]addFriendResponseWithMid:[AccountManager sharedAccountManager].loginModel.mid friend:model.mid opttype:@"1" complete:^(BaseModel *model) {
...
-(void)cellleftButtonTouch:(UIButton *)sender{
    ...
    [[ShareWork sharedManager]addFriendResponseWithMid:[AccountManager sharedAccountManager].loginModel.mid friend:model.mid opttype:@"2" complete:^(BaseModel *model) {
          [self initRefreshView];
    }];
