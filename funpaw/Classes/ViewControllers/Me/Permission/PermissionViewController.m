...
-(void)creatButtonTouch{
    if (self.dataSource.count >= 3) {
              [[AppUtil appTopViewController] showHint:@"规则不能超过3条哦"];
    }else{
...
-(void)setupData{
    [super setupData];
    [[ShareWork sharedManager]queryRuleWithMid:[AccountManager sharedAccountManager].loginModel.mid complete:^(BaseModel *model) {
...
-(void)perleftButtonTouch:(UIButton *)sender{
    ...
    [[ShareWork sharedManager]ruleModifyStatusWithMid:[AccountManager sharedAccountManager].loginModel.mid rid:model.rid complete:^(BaseModel *model) {
...
(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            ...
            [[ShareWork sharedManager]ruleDelWithRid:model.rid complete:^(BaseModel *model) {
                if (model) {
                [[AppUtil appTopViewController] showHint:model.retDesc];
